...
def kpi_card(label, value_display, exact_display, bg=GREEN_DARK, unit=None):
    # Built as a single-line string deliberately: a multi-line f-string here
    # previously left a blank line in place of {unit_html} whenever `unit`
    # was omitted, and Markdown treats a blank line as the end of an HTML
    # block — so everything after it (the value/exact number) rendered as
    # literal text instead of HTML. A single line has no such blank-line or
    # indentation risk.
    unit_html = f'<p class="kpi-unit">{unit}</p>' if unit else ""
    html = (
        f'<div class="kpi-card" style="background-color:{bg};" title="{exact_display}">'
        f'<p class="kpi-label">{label}</p>'
        f'{unit_html}'
        f'<p class="kpi-value">{value_display}</p>'
        f'<p class="kpi-exact">Exact: {exact_display}</p>'
        f'</div>'
    )
